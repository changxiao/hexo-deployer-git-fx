...
module.exports = function(args) {
  ...
  function setup(repo) {
    ...
    return fs.writeFile(pathFn.join(deployDir, 'placeholder'), '').then(function() {
      ...
      return pull(repo);
    }).catch(function() { // if git clone fail, init git
      return fs.emptyDir(deployDir).then(function() {
  ...
  function firstCommit() {
    return setLocalUserName().then(function() {
      return git('add', '-A');
    }).then(function() {
  ...
  function pull(repo) { // update remote, then checkout -b
    return git('remote', 'rm', 'origin').finally(function() {
  ...
  function checkout(repo) {
    return git('fetch', 'origin').then(function() {
      return git('checkout', repo.branch).catch(function() {
        return git('checkout', '-B', repo.branch);
      });
    }).catch(function() { // if checkout fail, create new branch.
      return git('branch', repo.branch).then(function() {
        return git('checkout', repo.branch);
      });
    });
  }

  function getUserName() { // get object(username,email) from file(config)
    if (args.configDir) {
      var userConfigDir = pathFn.join(baseDir, args.configDir);
      console.log('exists configDir, so overwrite username');
      return require(userConfigDir);
    }

    return false;
  }

  function setLocalUserName() { // set git local config
    var configUsername = getUserName();
    var userName = configUsername.username || args.name || args.user || args.userName || '';
    var userEmail = configUsername.email || args.email || args.userEmail || '';
  ...
  return fs.exists(deployDir).then(function(exist) {
    ...
    if (exist) {
      // return Promise.all(config.map(function(repo) {
      //   return pull(repo);
      // }));
      return fs.emptyDir(deployDir).then(function () {
        return setup(config);
    ...
    // 完成git操作后删除目录和本地git，防止动态修改配置文件中git地址时报错。
    // 缺点：效率降低，增加git clone流量。
    return fs.emptyDir(deployDir).then(function () {
      return fs.rmdir(deployDir);
    });
  });
};
